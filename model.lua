...
function Model:generateAnswers(dataloader, params, batch, iter)
    -- check decoder
	...
	local encInSeq = batch['ques_fwd']:t();
	...
	hiddenBeams[level]['output'] = self.encoder.rnnLayers[level].output[batch['ques_fwd']:size(2)][iter];
	hiddenBeams[level]['cell'] = self.encoder.rnnLayers[level].cell[batch['ques_fwd']:size(2)][iter];
